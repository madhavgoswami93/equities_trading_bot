...
class MeanRevBacktester:
    ...
    def set_parameters(self, SMA=None, dev=None):
        ...
        if SMA is not None:
            ...
            self.data["Lower"] = (
                self.data["SMA"] - self.data["price"].rolling(self.SMA).std() * self.dev
            )
            self.data["Upper"] = (
                self.data["SMA"] + self.data["price"].rolling(self.SMA).std() * self.dev
            )
...
if __name__ == "__main__":
    tester = MeanRevBacktester("RBLBANK", 30, 2, "2019-01-01", "2020-06-30", 0)
    ...
    # tester.plot_results()
    # tester.results[["creturns", "cstrategy"]].plot(
    #     title="MeanReversion Strategy", figsize=(12, 8)
    # )
